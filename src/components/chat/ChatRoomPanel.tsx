...
export type ChatModeracionAction = (
  id: string,
) => Promise<{ ok: boolean; error?: string; mensaje?: MensajeChatRealtimeRow }>;
...
interface ChatRoomPanelProps {
  ...
  esModerador: boolean;
  moderadorBadge?: string;
  canSend: boolean;
  inputPlaceholder: string;
  closedBadge?: string | null;
  headerHint?: string | null;
  onSend: (texto: string) => Promise<{ ok: boolean; error?: string }>;
  onReportar: ChatModeracionAction;
  onAprobar: ChatModeracionAction;
  onEliminar: ChatModeracionAction;
  realtime: {
    ligaId: string;
...
export function ChatRoomPanel({
  ...
  const appendMensaje = useCallback(
    (msg: MensajeChatConAutor) => {
      setMensajes((prev) => {
        ...
        if (msg.oculto && !esModerador) return prev;
        return [...prev, msg];
      });
    },
    [esModerador],
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (!matchesRealtimeRow(row)) return;
  ...
  async function sendMessage(trimmed: string) {
    return onSend(trimmed);
  }

  function runModeracion(mensajeId: string, action: ChatModeracionAction) {
    setError(null);
    ...
    <section className="flex min-h-[min(380px,45vh)] flex-1 flex-col overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/60">
        ...
        {mensajes.length === 0 ? (
          ...
          mensajes.map((msg) => (
            <MensajeBubble
              ...
              onReportar={() => runModeracion(msg.id, onReportar)}
              onAprobar={() => runModeracion(msg.id, onAprobar)}
              onEliminar={() => runModeracion(msg.id, onEliminar)}
            />
          ))
        )}
      ...
      >
        ...
        <div className="flex gap-2">
          ...
          <button
            type="submit"
            disabled={isPending || !texto.trim() || !canSend}
