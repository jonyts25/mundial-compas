/**
 * Match Preview — motor rule-based del Prediction Engine (El Pitoniso · PI-1).
 *
 * Calcula veredicto 1X2 recreativo a partir de multitud, tabla, forma y contexto.
 * Sin marca, sin copy, sin fetch, sin efectos.
 *
 * Reutiliza tipos de `pick-aggregates`; no recalcula shares.
 */
...
export interface MatchPreviewTeamInput {
  /** Posición en mini-tabla (1 = primero). null si no aplica. */
  tablePosition: number | null;
  /** Equipos en el grupo. null si no hay mini-tabla. */
  groupSize: number | null;
  /** Forma normalizada 0–1. null = sin historial (debut). */
  formNorm: number | null;
  /** Puntos de distancia al 2.º lugar (0 = dentro del top 2). null si N/A. */
...
export interface MatchPreviewVerdict {
  ...
  /** Señales no-multitud alineadas con el favorito (0–3). */
...
/**
 * Calcula el veredicto 1X2 recreativo. Pura: mismas entradas → misma salida.
 */
