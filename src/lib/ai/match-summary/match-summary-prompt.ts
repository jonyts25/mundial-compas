...
export function buildMatchSummaryPrompt(input: MatchSummaryInput): string {
  ...
  const eventTextBlock =
    input.timeline.length > 0
      ? `EVENTOS (usa event_text literal o parafraseo mínimo — NO reinterpretes):\n${input.timeline
          .map((e) => `- ${e.event_text ?? `${e.type} ${e.player}`}`)
          .join("\n")}`
  ...
  const verifiedFactsBlock =
    input.verified_facts && input.verified_facts.length > 0
      ? `HECHOS VERIFICADOS (generados por código — única fuente para facts[]):\n${input.verified_facts.map((f) => `- ${f}`).join("\n")}`
      : "";

  const guardBlock = `PROHIBIDO INFERIR (salvo narrative_evidence explícita en JSON):
- segunda amarilla, doble amonestación, roja directa
- penal polémico
- cambió el rumbo / volteó el partido
- confirmó el marcador / confirmando el marcador / selló la victoria
- dominó psicológicamente / controló emocionalmente
Puedes conectar eventos entre sí; NO reinterprete su naturaleza (ej. VAR Card upgrade NO es expulsión ni segunda amarilla).`;

  const namesBlock =
    input.facts_locked && input.facts_locked.length > 0
      ? `NOMBRES BLOQUEADOS (facts_locked — copiar EXACTAMENTE, sin expandir iniciales ni completar):
...
- facts[] = copia o parafraseo mínimo de verified_facts[]; no añadas inferencias.
