...
export async function syncLiveScoresFromApiSports(
  ...
  try {
    const staleIds = await fetchStaleLiveFixtureIds(supabase, liveIds);
    if (staleIds.length > 0) {
      const staleItems = await fetchApiSportsFixturesByIds(apiKey, staleIds, timezone);
      result.apiRequests += Math.ceil(staleIds.length / 20);
      result.fetched += staleItems.length;
