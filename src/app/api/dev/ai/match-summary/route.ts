...
export async function POST(request: Request) {
  ...
  const nameViolations = scanMatchSummaryNameViolations(
    match_summary_output,
    getLockedPlayerNames(input),
  );
  if (nameViolations.length > 0) {
    return NextResponse.json(
      {
        ok: false,
        error: formatNameGuardError(nameViolations),
        name_violations: nameViolations,
        input,
        match_summary_output,
      },
      { status: 422 },
